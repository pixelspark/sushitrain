                ...
                if targetEntry.isDirectory() {
                    ...
                    .contextMenu {
                        ...
                        NavigationStack {  // to force the image to take up all available space
                            VStack {
                                ThumbnailView(
                                    file: targetEntry, appState: appState, showFileName: false,
                                    showErrorMessages: false
                                )
                                .frame(
                ...
                if !isSearching {
                    ...
                    case .grid:
                        VStack {
                            ScrollView {
                                HStack {
                                    FolderStatusView(appState: appState, folder: folder).padding(
                                        .all, 10)
                                ...
                                GridFilesView(
                                    appState: appState, prefix: self.prefix, files: files,
                        ...
                        List {
                            ...
                            Section {
                                ForEach(files, id: \.self) { file in
                                    EntryView(
                                        appState: appState, entry: file, folder: folder,
                                        siblings: files,
                ...
                ToolbarItem {
                    Menu {
                        ...
                        if folderExists && folderIsSelective {
                            NavigationLink(
                                destination: SelectiveFolderView(appState: appState, folder: folder)
                            ) {
                ...
                ToolbarItem {
                    Menu(
                        content: {
                            ...
                            Toggle(
                                "Search here...", systemImage: "magnifyingglass", isOn: $showSearch
                            ).disabled(!folderExists)
                            ...
                            if folderExists && !self.prefix.isEmpty {
                                ...
                                {
                                    NavigationLink(
                                        destination: FileView(file: entry, appState: self.appState)
                                    ) {
                                        Label(
                                            "Subdirectory properties...",
                                            systemImage: "folder.badge.gearshape")
                            ...
                            Button(
                                "Folder settings...", systemImage: "folder.badge.gearshape",
                                action: {
                                    showSettings = true
                                }
                            ).disabled(!folderExists)
                        ...
                        label: {
                            Image(systemName: "ellipsis.circle").accessibilityLabel(Text("Menu"))
                        })
                }
            #endif
        }
