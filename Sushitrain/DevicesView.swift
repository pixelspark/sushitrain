            ...
            .toolbar {
                ToolbarItemGroup(placement: .primaryAction) {
                    Picker("View as", selection: $viewStyle) {
                        Image(systemName: "list.bullet").tag(DevicesViewStyle.list).accessibilityLabel(Text("List"))
                        Image(systemName: "tablecells").tag(DevicesViewStyle.grid).accessibilityLabel(Text("Grid"))
                    }
...
fileprivate struct DevicesListView: View {
    @ObservedObject var appState: AppState
                        ...
                        Table(self.peers, selection: $selectedPeers) {
                            ...
                            TableColumnForEach(self.folders) { folder in
                                TableColumn(folder.displayName) { device in
                                    DevicesGridCellView(appState: appState, device: device, folder: folder, viewStyle: viewStyle)
                                }
                                .width(ideal: 50)
                                .alignment(.center)
                            }
    ...
    fileprivate struct DevicesGridCellView: View {
        ...
        var body: some View {
            ...
            .sheet(isPresented: $showEditEncryptionPassword) {
                NavigationStack {
                    ShareFolderWithDeviceDetailsView(appState: self.appState, folder: self.folder, deviceID: .constant(device.deviceID()))
                }
            }
        }
