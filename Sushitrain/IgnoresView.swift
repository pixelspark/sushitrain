...
struct IgnoresView: View {
    ...
    private static let defaultIgnoreLines = ["(?d).DS_Store", "(?d)Thumbs.db", "(?d)desktop.ini", ".AppleDB", ".AppleDesktop", ".Trashes", ".Spotlight-V100"]
    
    var body: some View {
        List {
            Section {
                Text("This is an advanced feature and should only be used if you know what you are doing.")
            ...
            footer: {
                    Text("Files and subdirectories whose paths match any of the patterns above will not be synchronized with other devices. Existing items matching the patterns will not be updated. ")
                    #if os(macOS)
        ...
        .confirmationDialog("Are you sure you want to apply the patterns and to clean the folder? Files and subdirectories that match the patterns will be permanently removed from this device. Files may be lost if they are not available on other devices.", isPresented: $showUpdateCleanConfirmation, titleVisibility: .visible) {
